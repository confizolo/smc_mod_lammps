			...
			if len(equil):
				if do_relax:
					...
					if overwrite_accessible:
						new_file_name = os.path.join(os.path.dirname(master_molecule_file), os.path.basename(master_molecule_file) )
						master_molecule_file = generate_stiff(_n_stiff, equil, master_molecule_file, new_file_name, force = True, pattern = _pattern, overwrite_accessible=False)
